    ...
    def __next__(self):
        ...
        while not found:
            ...
            if self.iter_i[1] == len(self.iter_idx):
                if self.iter_i[0] == len(self.iter_idx):
        ...
        if self.iter_i[0] == self.iter_i[1]:
            return (self.iter_var, [self.iter_idx[self.iter_i[0]], self.iter_idx[self.iter_i[1]]],
                    self.df.iloc[:self.iter_idx[self.iter_i[0]]][self.class_var].values,
                    self.df.iloc[self.iter_idx[self.iter_i[0]]:][self.class_var].values)
